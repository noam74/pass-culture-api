...
def handle_expired_bookings() -> None:
    ...
    if settings.IS_STAGING:
        ...
        try:
            ...
            notify_users_of_expired_non_eac_bookings()
        except Exception as e:  # pylint: disable=broad-except
...
def notify_users_of_expired_non_eac_bookings(expired_on: datetime.date = None) -> None:
    expired_on = expired_on or datetime.date.today()

    logger.info("[notify_users_of_expired_bookings] Start")
    expired_bookings_ordered_by_user = bookings_repository.find_expired_non_eac_bookings_ordered_by_user(expired_on)
