...
class ReimbursementRateForBookAbove20000(payments_models.ReimbursementRule):
    ...
    def is_relevant(self, booking: Booking, cumulative_revenue: Decimal) -> bool:
        if non_eligible_type(booking.stock.offer):
            return False
        if _is_educational_offer(booking.stock.offer):
            return False
        return cumulative_revenue > 20000
...
def _irrevelant_for_gradual_decreasing(offer: Offer) -> bool:
    return _is_educational_offer(offer) or offer.product.isDigital


def non_eligible_type(offer: Offer) -> bool:
    return not offer.type == str(ThingType.LIVRE_EDITION)


def _is_educational_offer(offer: Offer) -> bool:
    return offer.isEducational
